...
class Parser:
    """This class parsing the VM File
    By loading the given VM file and provides helper functions to parse the commands.
    """
    ...
    def arg1(self):
        ...
        For C_ARITHMETIC returns the command itself. Should not be called for C_RETURN."""
    ...
    def arg2(self):
        ...
        Only valid for C_PUSH, C_POP, C_FUNCTION, and C_RETURN."""
...
class CodeWriter:
    ...
    def perform_binary_arithmetic_command(self,output,  command):
        # Pop Stack into D.
        output.append("@SP")
    ...
    def perform_unary_arithmetic_command(self, output, command):
        # Access to Stack[-1]
        output.append("@SP")
        output.append("A=M-1")
        output.append(self.symbols[command])

    def perform_comparison_command(self, output, command):
    ...
    def write_arithmetic(self, command):
        ...
        if command in ["add", "sub", "and", "or"]:
            self.perform_binary_arithmetic_command(output, command)
        elif command in ["neg", "not"]:
            self.perform_unary_arithmetic_command(output, command)
        elif command in ["eq", "gt", "lt"]:
            self.perform_comparison_command(output, command)
        else:
    ...
    def perform_push_command(self, output, segment, index):
        if segment == "constant":
    ...
    def perform_pop_command(self, output, segment, index):
        if segment == "constant":
            # Not a valid command.
    ...
    def write_push_pop(self, command, segment, index):
        ...
        output = []
        if command == "C_PUSH":
            self.perform_push_command(output, segment, index)
        elif command == "C_POP":
            self.perform_pop_command(output, segment, index)
        else:
            raise NameError("Unexpected Command Type")
...
def parse_single_command(parser, code_writer):
    parser.advance()
...
def main():
    ...
    while parser.has_more_commands():
        parse_single_command(parser, code_writer)
